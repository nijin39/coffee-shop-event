...
export const lambdaHandler: <TContext extends Context>(
  event: RouterEvent,
  context: TContext
) => Promise<any> = router.handler({
  proxyIntegration: {
    routes: [
      {
        ...
        action: async (request, context) => {
          return (await barcodeController.createBarcode(
            request.paths?.customerId
      ...
      },
      {
        ...
        action: async (request, context) => {
          const response: LambdaResponse =
            await barcodeController.getBarcodeInfo(request.paths?.customerId);
      ...
      },
      {
        ...
        action: async (request, context) => {
          const response: LambdaResponse =
            await barcodeController.deleteBarcode(request.paths?.customerId);
          return response as ProxyIntegrationResult;
        },
      },
      {
      ...
      },
      {
        ...
        action: async (request, context) => {
          const response: LambdaResponse =
            await customerController.getCustomerInfo(request.paths?.customerId);
      ...
      },
      {
        ...
        action: async (request, context) => {
          const response: LambdaResponse =
            await customerController.getGiftCount(request.paths?.customerId);
          return response as ProxyIntegrationResult;
        },
      },
      {
        ...
        action: async (request, context) => {
          const response: LambdaResponse =
            await customerController.createCustomerInfo(
              request as ProxyIntegrationEvent<CreateCustomerCommand>
            );
